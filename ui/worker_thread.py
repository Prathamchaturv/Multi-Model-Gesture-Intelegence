...
def _load_voice_control_settings(config: Config) -> dict:
    ...
    settings = {
        ...
        'ambient_calibration_s': float(config.get('voice_control.ambient_calibration_s', 0.6)),
        'noise_reduction_enabled': bool(config.get('voice_control.noise_reduction_enabled', True)),
        'noise_gate_rms': float(config.get('voice_control.noise_gate_rms', 12.0)),
        'recognition_max_retries': int(config.get('voice_control.recognition_max_retries', 2)),
        'retry_backoff_s': float(config.get('voice_control.retry_backoff_s', 0.2)),
        'adaptive_confidence_enabled': bool(config.get('voice_control.adaptive_confidence_enabled', True)),
        'confidence_penalty_per_retry': float(config.get('voice_control.confidence_penalty_per_retry', 0.08)),
    ...
    def run(self) -> None:  # noqa: C901  (unavoidably long entry point)
        ...
        try:
            ...
            voice_listener = VoiceCommandListener(
                ...
                ambient_calibration_s=float(voice_cfg.get('ambient_calibration_s', 0.6)),
                noise_reduction_enabled=bool(voice_cfg.get('noise_reduction_enabled', True)),
                noise_gate_rms=float(voice_cfg.get('noise_gate_rms', 12.0)),
                recognition_max_retries=int(voice_cfg.get('recognition_max_retries', 2)),
                retry_backoff_s=float(voice_cfg.get('retry_backoff_s', 0.2)),
                adaptive_confidence_enabled=bool(voice_cfg.get('adaptive_confidence_enabled', True)),
                confidence_penalty_per_retry=float(voice_cfg.get('confidence_penalty_per_retry', 0.08)),
                request_error_backoff_s=float(voice_cfg.get('request_error_backoff_s', 1.5)),
                max_request_error_backoff_s=float(voice_cfg.get('max_request_error_backoff_s', 8.0)),
